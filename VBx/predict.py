...
def get_embedding(
    fea, model, label_name=None, input_name=None, backend="pytorch", device=None
):
    if backend == "pytorch":
...
def load_model(
    ...
    label_name, input_name, input_shape = None, None, None
    ...
    if backend == "pytorch":
        if model_file is not None:
            ...
            raise ValueError(
                "For pytorch backend, provide --model/--weights or --model-file"
            )
    elif backend == "onnx":
    ...
    return model, label_name, input_name, device, input_shape


def extract_xvectors_melbank(
    file_names,
    wav_dir,
    lab_dir,
    out_ark_fn,
                ...
                for fn in file_names:
                    with Timer(f"Processing file {fn}"):
                        ...
                        labs = np.atleast_2d(
                            (
                                np.loadtxt(
                                    f"{os.path.join(lab_dir, fn)}.lab", usecols=(0, 1)
                                )
                                * samplerate
                            ).astype(int)
                        )
...
def extract_xvectors_pcm(
    ...
    input_name,
    seg_len_samples: int,
    seg_jump_samples: int,
    device="cpu",
    backend="onnx",
):
    assert backend == "onnx", "Only ONNX supported for now for PCM mode"
        ...
        with open(out_ark_fn, "wb") as ark_file:
            for fn in file_names:
                with Timer(f"Processing file {fn}"):
                    ...
                    for segnum in range(len(labs)):
                        seg = signal[labs[segnum, 0] : labs[segnum, 1]]
                        if seg.shape[0] < seg_len_samples:
                            continue

                        seg_offset_s = labs[segnum, 0] / float(samplerate)
                        ...
                        for start in range(
                            ...
                            if np.isnan(xvector).any():
                                ...
                                seg_start = round(
                                    seg_offset_s + start / float(samplerate), 3
                                )
                                seg_end = round(
                                    seg_offset_s
                                    + (start + seg_len_samples) / float(samplerate),
                                    3,
                                )
                                seg_file.write(
                                    f"{key} {fn} {seg_start} {seg_end}{os.linesep}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpus", type=str, default="", help="use gpus (passed to CUDA_VISIBLE_DEVICES)"
    )
    parser.add_argument(
    ...
    if args.use_pcm:
        ...
        extract_xvectors_pcm(
            ...
            label_name,
            input_name,
            seg_len_samples=input_shape[1],
            seg_jump_samples=args.seg_jump,
            device=device,
            backend=args.backend,
        )
    else:
